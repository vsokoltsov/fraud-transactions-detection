...
def pr_at_thresholds(
    thr_list: List[np.float64],
    thresholds: List[np.float64],
    precision: npt.NDArray[np.float64],
    recall: npt.NDArray[np.float64]
) -> List[Tuple[np.float64, np.float64, np.float64, np.int64]]:
    pts = []
    for t in thr_list:
...
def save_features(filename: str, features: List[str]) -> None:
    with open(filename, "w", encoding="utf-8") as f:
        yaml.dump(features, f, allow_unicode=True, sort_keys=False)

def save_dataframes(dfs: Dict[str, pd.DataFrame]) -> pd.DataFrame:
