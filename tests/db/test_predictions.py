...
class TestPredictions:
    ...
    def test_generate_features_calls_all_functions(
        self, sample_dataframe: pd.DataFrame
    ) -> None:
        """Test that generate_features calls all feature engineering functions."""
        with (
            patch("api.db.predictions.tx_amount_columns") as mock_tx,
    ...
    def test_tx_amount_columns_basic_functionality(
        self, sample_dataframe: pd.DataFrame
    ) -> None:
    ...
    def test_tx_amount_columns_groupby_calculations(
        self, sample_dataframe: pd.DataFrame
    ) -> None:
    ...
    def test_time_since_prev_trx_basic_functionality(
        self, sample_dataframe: pd.DataFrame
    ) -> None:
    ...
    def test_time_since_prev_trx_calculation_accuracy(
        self, single_customer_dataframe: pd.DataFrame
    ) -> None:
        ...
        expected_diff_1 = (
            pd.Timestamp("2023-01-01 11:00:00") - pd.Timestamp("2023-01-01 10:00:00")
        ).total_seconds()
        expected_diff_2 = (
            pd.Timestamp("2023-01-01 12:00:00") - pd.Timestamp("2023-01-01 11:00:00")
        ).total_seconds()

        assert result.iloc[1]["secs_since_prev_tx"] == expected_diff_1
        assert result.iloc[2]["secs_since_prev_tx"] == expected_diff_2

    def test_burst_deviation_basic_functionality(
        self, sample_dataframe: pd.DataFrame
    ) -> None:
    ...
    def test_rs_anomaly_basic_functionality(
        self, sample_dataframe: pd.DataFrame
    ) -> None:
    ...
    def test_hour_deviates_basic_functionality(
        self, sample_dataframe: pd.DataFrame
    ) -> None:
    ...
    def test_hour_deviates_weekend_calculation(
        self, sample_dataframe: pd.DataFrame
    ) -> None:
        ...
        weekend_df = pd.DataFrame(
            {
                ...
                "tx_datetime": pd.to_datetime(
                    ["2023-01-07 10:00:00", "2023-01-08 11:00:00"]
                ),  # Saturday and Sunday
    ...
    def test_robust_amount_outlier_basic_functionality(
        self, sample_dataframe: pd.DataFrame
    ) -> None:
    ...
    def test_robust_amount_outlier_sorting(
        self, sample_dataframe: pd.DataFrame
    ) -> None:
        ...
        assert result.groupby("customer_id")[
            "tx_datetime"
        ].is_monotonic_increasing.all()

    def test_day_of_week_zscore_outlier_basic_functionality(
        self, sample_dataframe: pd.DataFrame
    ) -> None:
    ...
    def test_fraud_burst_candidate_basic_functionality(
        self, sample_dataframe: pd.DataFrame
    ) -> None:
    ...
    def test_fraud_burst_candidate_24h_calculation(
        self, single_customer_dataframe: pd.DataFrame
    ) -> None:
    ...
    def test_dataframe_modification_behavior(
        self, sample_dataframe: pd.DataFrame
    ) -> None:
