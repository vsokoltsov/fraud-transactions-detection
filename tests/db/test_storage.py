...
class TestStorage:
    ...
    def test_prepare_for_verification_delegates_to_store(
        self, mock_settings: Mock, mock_csv_storage: Mock
    ) -> None:
        ...
        with patch("api.db.storage.CSVStorage", return_value=mock_csv_storage):
            ...
            mock_csv_storage.prepare_for_verification.assert_called_once_with(
                trx_id=5, features=features
            )
            pd.testing.assert_frame_equal(
                result, mock_csv_storage.prepare_for_verification.return_value
            )
    ...
    def test_prepare_for_verification_with_different_trx_ids(
        ...
        with patch("api.db.storage.CSVStorage", return_value=mock_csv_storage):
            ...
            mock_csv_storage.prepare_for_verification.assert_any_call(
                trx_id=0, features=features
            )
            mock_csv_storage.prepare_for_verification.assert_any_call(
                trx_id=10, features=features
            )
            mock_csv_storage.prepare_for_verification.assert_any_call(
                trx_id=999, features=features
            )
    ...
    def test_storage_error_handling_from_underlying_store(
        ...
        with patch("api.db.storage.CSVStorage", return_value=mock_csv_storage):
            ...
            mock_csv_storage.prepare_for_verification.side_effect = IndexError(
            ...
            mock_csv_storage.prepare_for_verification.side_effect = Exception(
                "Storage error"
            )
    ...
    def test_storage_initialization_error_handling(self) -> None:
        ...
        with patch(
            "api.db.storage.CSVStorage", side_effect=FileNotFoundError("File not found")
        ):
