...
class TestFraudPredictor:
    ...
    def test_init_linear_regression(self, mock_settings: Mock, mock_features: list[str], mock_thresholds: dict[str, float]) -> None:
    ...
    def test_init_random_forest(self, mock_settings: Mock, mock_features: list[str], mock_thresholds: dict[str, float]) -> None:
    ...
    def test_init_xgboost(self, mock_settings: Mock, mock_features: list[str], mock_thresholds: dict[str, float]) -> None:
    ...
    def test_model_path_construction(self, mock_settings: Mock, mock_features: list[str], mock_thresholds: dict[str, float]) -> None:
    ...
    def test_xgboost_model_path_construction(self, mock_settings: Mock, mock_features: list[str], mock_thresholds: dict[str, float]) -> None:
    ...
    def test_all_model_types_initialization(self, mock_settings: Mock, model_type: str, mock_features: list[str], mock_thresholds: dict[str, float]) -> None:
    ...
    def test_load_features(self, mock_settings: Mock, mock_features: list[str]) -> None:
        ...
        with patch('yaml.safe_load', return_value=mock_features), \
             patch('builtins.open', create=True) as mock_open:
            
            result = predictor._load_features()
            
            assert result == mock_features
            mock_open.assert_called_once_with(mock_settings.features_path, "r", encoding="utf-8")

    def test_load_model_thresholds(self, mock_settings: Mock, mock_thresholds: dict[str, float]) -> None:
        """Test _load_model_thresholds method."""
        predictor = FraudPredictor.__new__(FraudPredictor)
        predictor.settings = mock_settings
        
        with patch('yaml.safe_load', return_value=mock_thresholds), \
             patch('builtins.open', create=True) as mock_open:
            
            result = predictor._load_model_thresholds("/fake/thresholds.yaml")
            
            assert result == mock_thresholds
            mock_open.assert_called_once_with("/fake/thresholds.yaml", "r", encoding="utf-8")

    def test_threshold_for_model(self, mock_settings: Mock, mock_thresholds: dict[str, float]) -> None:
        """Test _threshold_for_model method."""
        predictor = FraudPredictor.__new__(FraudPredictor)
        predictor.settings = mock_settings
        
        with patch.object(predictor, '_load_model_thresholds', return_value=mock_thresholds):
    ...
    def test_threshold_for_model_not_found(self, mock_settings: Mock) -> None:
        ...
        with patch.object(predictor, '_load_model_thresholds', return_value={'linear_regression': 0.5}):
            with pytest.raises(ValueError, match="There is no threshold for model 'unknown_model'"):
    ...
    def test_features_loading_error(self, mock_settings: Mock) -> None:
        ...
        with patch("builtins.open", side_effect=FileNotFoundError("Features file not found")), \
            patch("joblib.load"):  # чтобы не мешал
            with pytest.raises(FileNotFoundError, match="Features file not found"):
                FraudPredictor(mock_settings)

    def test_thresholds_loading_error(self, mock_settings: Mock, mock_features: list[str]) -> None:
        ...
        with patch('joblib.load') as mock_load, \
            ...
            mock_yaml_load.side_effect = [mock_features, FileNotFoundError("Thresholds file not found")]
