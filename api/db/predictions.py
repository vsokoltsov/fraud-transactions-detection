...
def rs_anomaly(df: pd.DataFrame) -> pd.DataFrame:
    df["zscore"] = (
        df["tx_amount_log"]
        - df.groupby("customer_id")["tx_amount_log"].transform("mean")
    ) / df.groupby("customer_id")["tx_amount_log"].transform("std")
...
def hour_deviates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["hour_zscore"] = (
        df["hour"] - df.groupby("customer_id")["hour"].transform("mean")
    ) / df.groupby("customer_id")["hour"].transform("std")
...
def day_of_week_zscore_outlier(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["day_of_week_zscore"] = (
        df["day_of_week"] - df.groupby("customer_id")["day_of_week"].transform("mean")
    ) / df.groupby("customer_id")["day_of_week"].transform("std")
